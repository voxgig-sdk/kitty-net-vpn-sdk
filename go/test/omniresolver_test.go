...
import (
	...
	omni "github.com/voxgig-sdk/kitty-net-vpn-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.KittyNetVpnError); is {
				out := map[string]any{
					"name":    "KittyNetVpnError",
...
func omniCtx(arg any, client *sdk.KittyNetVpnSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.KittyNetVpnSDK); is {
			client = live
