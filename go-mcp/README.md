...
[MCP](https://modelcontextprotocol.io) server exposing the KittyNetVpn SDK as
two agent tools — `kitty-net-vpn_list` and `kitty-net-vpn_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kitty-net-vpn-mcp)
make build

# 2. Provide credentials via the environment
export KITTY_NET_VPN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user kitty-net-vpn \
  -- /absolute/path/to/kitty-net-vpn-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./kitty-net-vpn-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// kitty-net-vpn_list: first page of records
{ "entity": "server" }
{ "entity": "server", "query": { } }

// kitty-net-vpn_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kitty-net-vpn-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user kitty-net-vpn \
     -- "$PWD"/dist/*/kitty-net-vpn-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `kitty-net-vpn_list` and `kitty-net-vpn_load` tools now appear
   in new sessions. Ask the agent to *"list server using kitty-net-vpn"*
   and it calls `kitty-net-vpn_list` with `{"entity":"server"}`.
...
```sh
export KITTY_NET_VPN_APIKEY=sk_live_xxx            # API key
export KITTY_NET_VPN_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `kitty-net-vpn_list` tool
...
### Call the `kitty-net-vpn_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `kitty-net-vpn_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `kitty-net-vpn_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `KITTY_NET_VPN_APIKEY` | API key sent with every request. |
| `KITTY_NET_VPN_BASE` | Optional override of the API base URL. |
...
```sh
./kitty-net-vpn-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"kitty-net-vpn_load","arguments":{"entity":"server","query":{"id":1}}}}'
```
