...
class KittyNetVpnTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            KittyNetVpnTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = KittyNetVpnTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = KittyNetVpnTestRunner.getenv("KITTY_NET_VPN_TEST_LIVE")
        override = KittyNetVpnTestRunner.getenv("KITTY_NET_VPN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = KittyNetVpnTestRunner.getenv(key)
        ...
        explain = KittyNetVpnTestRunner.getenv("KITTY_NET_VPN_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if KittyNetVpnTestRunner._test_control is not None:
            return KittyNetVpnTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                KittyNetVpnTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            KittyNetVpnTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = KittyNetVpnTestRunner.load_test_control()
...
def load_env_local():
    KittyNetVpnTestRunner.load_env_local()


def env_override(m):
    return KittyNetVpnTestRunner.env_override(m)


def entity_data(v):
    return KittyNetVpnTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return KittyNetVpnTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return KittyNetVpnTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return KittyNetVpnTestRunner.load_test_control()


def live_client_options():
    return KittyNetVpnTestRunner.live_client_options()


def live_delay_ms():
    return KittyNetVpnTestRunner.live_delay_ms()
