...
module KittyNetVpnOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if KittyNetVpnOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = KittyNetVpnOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| KittyNetVpnOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || KittyNetVpnOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = KittyNetVpnSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = KittyNetVpnError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { KittyNetVpnOmni.errify(err) }
