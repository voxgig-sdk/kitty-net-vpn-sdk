...
class ServerEntity
  ...
  def data_set(args)
    if args
      @_data = KittyNetVpnHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = KittyNetVpnHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Server>, Array] the matching Server items; raises KittyNetVpnError on failure
