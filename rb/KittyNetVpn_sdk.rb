...
require_relative 'KittyNetVpn_types'


class KittyNetVpnSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = KittyNetVpnHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = KittyNetVpnHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, KittyNetVpnFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    KittyNetVpnUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = KittyNetVpnHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = KittyNetVpnHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = KittyNetVpnHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => KittyNetVpnError.new(
        "#{op}_allow",
        "KittyNetVpnSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = KittyNetVpnHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = KittyNetVpnError.new(
        "graphql_error", "KittyNetVpnSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = KittyNetVpnSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
