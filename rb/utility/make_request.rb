...
module KittyNetVpnUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = KittyNetVpnResponse.new({})
    ...
    if fetch_err
      ...
      response = KittyNetVpnResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = KittyNetVpnResponse.new(fetched)
    else
