// Command KittyNetVpn-cli is a CLI + REPL for the KittyNetVpn SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      KITTY_NET_VPN_APIKEY (api key), KITTY_NET_VPN_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: KITTY_NET_VPN_APIKEY carries the API key and
	// KITTY_NET_VPN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("KITTY_NET_VPN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("KITTY_NET_VPN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewKittyNetVpnSDK(opts)
