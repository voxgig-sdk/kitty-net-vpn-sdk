...
boru-driven command-line client **and** interactive REPL for the KittyNetVpn
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kitty-net-vpn-cli)
make build

# 2. See usage (words, entities, env vars)
./kitty-net-vpn-cli --help

# 3. Provide credentials once, via the environment
export KITTY_NET_VPN_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./kitty-net-vpn-cli list server

# 5. Override the API base URL for a single call
KITTY_NET_VPN_BASE=https://api.example.com ./kitty-net-vpn-cli list server

# 6. No arguments -> interactive REPL
./kitty-net-vpn-cli
kitty-net-vpn> list server
kitty-net-vpn> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kitty-net-vpn-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/kitty-net-vpn-cli list server
   ```
...
Run with no arguments to open a REPL (prompt `kitty-net-vpn>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/kitty-net-vpn-cli`. |
...
(`./kitty-net-vpn-cli <expr>`) and interactively (the REPL), and expressions compose the
