...
function isRunnable(code) {
    return (/new\s+KittyNetVpnSDK\b/.test(code) ||
        /\bKittyNetVpnSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+KittyNetVpnSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/KittyNetVpnSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
