...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.KITTYNETVPN_TEST_LIVE ||
    'TRUE' === process.env.KITTYNETVPN_TEST_OVERRIDE
  ) {
  ...
  m.KITTYNETVPN_TEST_EXPLAIN = process.env.KITTYNETVPN_TEST_EXPLAIN || m.KITTYNETVPN_TEST_EXPLAIN
