...
class FtCtrl extends KittyNetVpnControl
{
...
class FtClient
{
    ...
    private KittyNetVpnUtility $utility;

    public function __construct(KittyNetVpnUtility $utility, array $options)
    {
    ...
    public function get_utility(): KittyNetVpnUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (KittyNetVpnContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public KittyNetVpnContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (KittyNetVpnContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = KittyNetVpnConfig::make_feature($name);
    ...
    private static function buildUrl(KittyNetVpnSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof KittyNetVpnError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new KittyNetVpnResponse($fetched);
        }
    ...
    private function fail(KittyNetVpnContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (KittyNetVpnContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
