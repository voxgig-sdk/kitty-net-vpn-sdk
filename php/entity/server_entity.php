...
class ServerEntity
{
    ...
    {
        if ($args) {
            $this->_data = KittyNetVpnHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = KittyNetVpnHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws KittyNetVpnError on failure (item-5 convention).
     */
